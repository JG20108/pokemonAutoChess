...
function Game() {
    ...
    const spectateTillTheEnd = () => {
        ...
        if (gameContainer.gameScene) {
            ...
            (_b = (_a = gameContainer.gameScene) === null || _a === void 0 ? void 0 : _a.board) === null || _b === void 0 ? void 0 : _b.renderBoard();
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (!connected.current) {
            ...
            room.onMessage(types_1.Transfer.UNOWN_WANDERING, ({ id, pkm }) => {
            ...
            room.onMessage(types_1.Transfer.POKEMON_WANDERING, (data) => {
                const scene = getGameScene();
                const { id, pkm } = data;
                if (scene) {
                    (0, pokemon_1.addWanderingPokemon)(scene, id, pkm, (sprite, id, pointer, tween) => {
                        if (scene.board &&
                            (0, board_1.getFreeSpaceOnBench)(scene.board.player.board) > 0) {
                            room.send(types_1.Transfer.POKEMON_WANDERING, { id });
                            sprite.destroy();
                            tween.destroy();
                        }
                        else if (scene.board) {
            ...
            const $ = (0, colyseus_js_1.getStateCallbacks)(room);
