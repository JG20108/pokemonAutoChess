    ...
    execute() {
        ...
        if (this.pokemon.effects.has(Effect_1.Effect.RISING_VOLTAGE) ||
            ...
            if (isTripleAttack) {
                ...
                if (isPowerSurge) {
                    this.board
                        .getAdjacentCells(this.target.positionX, this.target.positionY, true)
                        .forEach((cell) => {
                        if (cell) {
                            const enemy = this.board.getValue(cell.x, cell.y);
                            if (enemy && this.pokemon.team !== enemy.team) {
                                enemy.handleSpecialDamage(10, this.board, Game_1.AttackType.SPECIAL, this.pokemon, false, false);
