...
class Simulation extends schema_1.Schema {
    ...
    applyEffect(pokemon, types, effect, activeSynergies) {
        switch (effect) {
            ...
            case Effect_1.Effect.GOOGLE_SPECS:
                if (types.has(Synergy_1.Synergy.ARTIFICIAL) && pokemon.items.size > 0) {
                    ...
                    const attackBoost = {
                        ...
                        [Effect_1.Effect.LINK_CABLE]: (6 / 100) * pokemon.baseAtk,
                        [Effect_1.Effect.GOOGLE_SPECS]: (12 / 100) * pokemon.baseAtk
                    }[effect];
                    const apBoost = {
                        [Effect_1.Effect.DUBIOUS_DISC]: 0,
                        [Effect_1.Effect.LINK_CABLE]: 6,
                        [Effect_1.Effect.GOOGLE_SPECS]: 12
                    }[effect];
                    const shieldBoost = {
                        [Effect_1.Effect.DUBIOUS_DISC]: 0,
                        [Effect_1.Effect.LINK_CABLE]: (6 / 100) * pokemon.hp,
                        [Effect_1.Effect.GOOGLE_SPECS]: (12 / 100) * pokemon.hp
                    }[effect];
            ...
            case Effect_1.Effect.CURSE_OF_VULNERABILITY:
            case Effect_1.Effect.CURSE_OF_WEAKNESS:
            case Effect_1.Effect.CURSE_OF_TORMENT:
        ...
        if ((redWaterLevel > 0 && this.tidalWaveCounter === 1) ||
            ...
            for (let y = 0; y < this.board.rows; y++) {
                for (let x = 0; x < this.board.columns; x++) {
                    ...
                    if (pokemonHit) {
                        if (pokemonHit.team === Game_1.Team.RED_TEAM) {
                            pokemonHit.status.clearNegativeStatus();
                            if (pokemonHit.types.has(Synergy_1.Synergy.AQUATIC)) {
                                pokemonHit.handleHeal(redWaterLevel * 0.1 * pokemonHit.hp, pokemonHit, 0, false);
                            }
                        }
                        else {
        ...
        if ((blueWaterLevel > 0 && this.tidalWaveCounter === 1) ||
            ...
            for (let y = this.board.rows - 1; y > 0; y--) {
                for (let x = 0; x < this.board.columns; x++) {
                    ...
                    this.board.effects[y * this.board.columns + x] = undefined;
                    if (pokemonHit) {
                        if (pokemonHit.team === Game_1.Team.BLUE_TEAM) {
                            pokemonHit.status.clearNegativeStatus();
                            if (pokemonHit.types.has(Synergy_1.Synergy.AQUATIC)) {
                                pokemonHit.handleHeal(blueWaterLevel * 0.1 * pokemonHit.hp, pokemonHit, 0, false);
                            }
                        }
                        else {
                            pokemonHit.handleDamage({
