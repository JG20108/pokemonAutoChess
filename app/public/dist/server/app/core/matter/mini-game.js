...
class MiniGame {
    ...
    pickRandomItems(state) {
        ...
        if (encounter === town_encounters_1.TownEncounters.XATU) {
            itemsSet = [Item_1.Item.WONDER_BOX];
            nbItemsToPick = this.alivePlayers.length;
            maxCopiesPerItem = 99;
        }
        if (encounter === town_encounters_1.TownEncounters.DUSKULL) {
            itemsSet = [Item_1.Item.GIMMIGHOUL_COIN];
            nbItemsToPick = this.alivePlayers.length;
            maxCopiesPerItem = 99;
        }
        if (encounter === town_encounters_1.TownEncounters.WOBBUFFET) {
            itemsSet = [Item_1.Item.EXCHANGE_TICKET];
            nbItemsToPick = this.alivePlayers.length;
            maxCopiesPerItem = 99;
        }
        if (encounter === town_encounters_1.TownEncounters.MUNCHLAX) {
            itemsSet = [Item_1.Item.PICNIC_SET];
            nbItemsToPick = this.alivePlayers.length;
            maxCopiesPerItem = 99;
        }
