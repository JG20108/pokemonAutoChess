...
class PokemonState {
    ...
    attack(pokemon, board, target) {
        if (target.life > 0) {
            ...
            if (pokemon.effects.has(Effect_1.Effect.CHARGE)) {
        ...
        if (pokemon.life === 0) {
            ...
            if (!pokemon.life || pokemon.life <= 0) {
                if (pokemon.hasSynergyEffect(Synergy_1.Synergy.FOSSIL)) {
                    ...
                    effects_1.SynergyEffects[Synergy_1.Synergy.FOSSIL].forEach((e) => pokemon.effects.delete(e));
                }
    ...
    getNearestAlly(pokemon, board) {
        let nearestAlly = undefined;
        let minDistance = 999;
        board.forEach((x, y, value) => {
            if (value && value.team === pokemon.team && pokemon.id !== value.id) {
                const distance = (0, distance_1.distanceM)(pokemon.positionX, pokemon.positionY, value.positionX, value.positionY);
