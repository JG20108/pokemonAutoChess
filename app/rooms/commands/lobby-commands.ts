    ...
    try {
      ...
      if (pokemonCollectionItem) {
        ...
        if (
          ...
          const mongoUser = await UserMetadata.findOne({
            uid: client.auth.uid,
          });
      ...
      const mongoUser = await UserMetadata.findOneAndUpdate(
        {
          ...
          $and: [
            ...
            {
              [`pokemonCollection.${index}.${
                shiny ? 'shinyEmotions' : 'emotions'
              }`]: { $ne: emotion },
            },
          ],
        },
...
> {
  async execute({
    ...
    try {
      ...
      logger.debug(
        `Creating tournament lobbies for tournament ${tournamentId}`
      );
      const tournament = this.state.tournaments.find(
        (t) => t.id === tournamentId
      );
      ...
      for (const bracket of brackets) {
        ...
        logger.info(
          `Creating tournament game ${bracket.name} id: ${bracketId}`
        );
  ...
  async execute({
    ...
    try {
      ...
      if (
        ...
        const mongoTournament = await Tournament.findById(tournamentId);
        if (mongoTournament) {
          mongoTournament.players = convertSchemaToRawObject(
            tournament.players
          );
          mongoTournament.brackets = convertSchemaToRawObject(
            tournament.brackets
