...
const getEnvVar = (key) => {
  ...
  return JSON.stringify(value || ''); // Ensure we always return a string, even if empty
};
...
context({
  ...
  target: 'es2016',
  define: {
    'process.env.FIREBASE_API_KEY': getEnvVar('FIREBASE_API_KEY'),
    'process.env.FIREBASE_AUTH_DOMAIN': getEnvVar('FIREBASE_AUTH_DOMAIN'),
    'process.env.FIREBASE_PROJECT_ID': getEnvVar('FIREBASE_PROJECT_ID'),
    'process.env.FIREBASE_STORAGE_BUCKET': getEnvVar('FIREBASE_STORAGE_BUCKET'),
    'process.env.FIREBASE_MESSAGING_SENDER_ID': getEnvVar(
      'FIREBASE_MESSAGING_SENDER_ID'
    ),
    'process.env.FIREBASE_APP_ID': getEnvVar('FIREBASE_APP_ID'),
    'process.env.DISCORD_SERVER': getEnvVar('DISCORD_SERVER'),
    'process.env.MIN_HUMAN_PLAYERS': getEnvVar('MIN_HUMAN_PLAYERS'),
  },
})
  .then((context) => {
    if (isDev) {
